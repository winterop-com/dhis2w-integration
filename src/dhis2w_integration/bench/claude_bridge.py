...
Usage:
    uv run python -m dhis2w_integration.bench.claude_bridge                 # session-default model, all rounds
    uv run python -m dhis2w_integration.bench.claude_bridge opus sonnet     # compare models
    uv run python -m dhis2w_integration.bench.claude_bridge --runs 3        # repeat the flaky composite round
"""
...
from dhis2w_integration.bench.bridge import READ_TASKS, REPO, WRITE_TASK, _normalize_args, _score_read, _used
from dhis2w_integration.bench.composite import SCENARIOS, _cleanup_model, _verify_model, run_cli, run_cli_json
